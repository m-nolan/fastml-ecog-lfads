...
def read_multiband_data(data_file_path,n_block):
    ...
    data_basename = os.path.basename(data_file_path)
    if n_block > 1:
        filt_data_file = data_basename[:7] + f'nband{n_block}_' + data_basename[7:]
        filt_data_file_path = os.path.join(data_dir_path,filt_data_file)
    else:
        filt_data_file_path = data_file_path
