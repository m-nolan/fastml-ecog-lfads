...
class LFADS_Multiblock_Net(nn.Module):
    ...
    def __init__(self,input_size, factor_size = 4,
                 ...
                 g_latent_size   = 64, u_latent_size  = 1,
                 controller_size = 64, generator_size = 64,
                 n_block = 2, fix_out_mix = True, mix_weight = None,
                 prior = {'g0' : {'mean' : {'value': 0.0, 'learnable' : True},
                                  'var'  : {'value': 0.1, 'learnable' : False}},
                          'u'  : {'mean' : {'value': 0.0, 'learnable' : False},
                                  'var'  : {'value': 0.1, 'learnable' : True},
        ...
        if fix_out_mix:
            if mix_weight == None:
                self.mix_weight = torch.ones(self.n_block)/self.n_block # replace with uniform def
            else:
                assert len(mix_weight) == self.n_block, "mix_weight must be the same length as the number of model blocks"
                self.mix_weight = torch.tensor(mix_weight)
            # self.out_mix = nn.Linear(in_features = n_block * input_size, out_features = input_size, bias=False)
            self.out_mix.weight = torch.nn.Parameter(
                self.mix_weight.sqrt(), # put fixed weight values here.
                requires_grad = False
            )
            for p in self.out_mix.parameters():
                p.requires_grad = False
                # set the weight values to something power-appropriate, like .99 and .01 or whatever
...
class LFADS_Multiblock_Net_v2(nn.Module):
    '''
        Similar to the LFADS_Net, but implementing multiple encoder/generator pair RNN cells in parallel for multiband processing.
        LFADS blocks are passed thorugh a linear mixing layer instead of superimposed, as they are in v1.
    '''
    def __init__(self,input_size, factor_size = 4,
                 g_encoder_size  = 64, c_encoder_size = 64,
                 g_latent_size   = 64, u_latent_size  = 1,
        ...
        super(LFADS_Multiblock_Net_v2, self).__init__()
        ...
        self.lfads_blocks = nn.ModuleList(lfads_blocks)
        # create linear net to mix LFADS block outputs
        self.out_mix = nn.Linear(in_features=n_block*input_size, out_features=input_size, bias=False) # stack block outputs in a new dimension then mix along that axis then squeeze.
    ...
    def forward(self,src):
        ...
        for idx, lb in enumerate(self.lfads_blocks):
            _recon, _ = lb(src[idx])
            block_outputs.append(_recon['data'])
        pred = self.out_mix(torch.cat(block_outputs,dim=-1))
        recon = {}
        recon['data'] = pred
        # omit factor returns for now, return block_outputs later.
        return recon, torch.stack(block_outputs,dim=-1) # consider routing the latent states here instead of the block outputs
    ...
    def forward_all(self,src):
        ...
        recon['data'] = pred
        # omit factor returns for now, return block_outputs later.
        return recon, (None, torch.stack(block_outputs,dim=-1), block_gen) # I've got to find a better way to pack these elements together. Dict?
...
class LFADS_Multiblock_GenMix_Net(nn.Module):
    '''
        Similar to the LFADS_Net, but implementing multiple encoder/generator pair RNN cells in parallel for multiband processing.
        LFADS blocks are passed thorugh a linear mixing layer instead of superimposed, as they are in v1.
    '''
    def __init__(self,input_size, factor_size = 4,
        ...
        super(LFADS_Multiblock_GenMix_Net, self).__init__()
        ...
        self.lfads_blocks = nn.ModuleList(lfads_blocks)
        # create linear net to mix LFADS block outputs
        self.out_mix = nn.Linear(in_features=n_block*generator_size, out_features=input_size, bias=False) # stack block outputs in a new dimension then mix along that axis then squeeze.
    ...
    def forward(self,src):
        ...
        block_gen = []
        for idx, lb in enumerate(self.lfads_blocks):
            _recon, (_, _gen, _) = lb.forward_all(src[idx])
            block_outputs.append(_recon['data'])
            block_gen.append(_gen)
        block_outputs = torch.stack(block_outputs,dim=-1) # too much data to hold onto, overwriting
        pred = self.out_mix(torch.cat(block_gen,dim=-1).permute(1,0,2))
        recon = {}
        recon['data'] = pred
        # omit factor returns for now, return block_outputs later.
        return recon, block_outputs # consider routing the latent states here instead of the block outputs
...
class LFADS_Ecog_SingleSession_Net(LFADS_Net):
    ...
    def forward(self, input):
        if len(input.shape) != 3: #[batch_size, seq_len, n_ch]
            breakpoint()
